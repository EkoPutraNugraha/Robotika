...
# Inisialisasi lidar
lidar = robot.getDevice("lidar")  # Nama perangkat lidar, pastikan benar
lidar.enable(TIME_STEP)
lidar.enablePointCloud()

# Inisialisasi sensor jarak (ultrasonic)
us = [robot.getDevice("us0"), robot.getDevice("us1")]  # Nama perangkat sensor ultrasonik
for sensor in us:
    sensor.enable(TIME_STEP)

# Inisialisasi motor
left_motor = robot.getDevice("left wheel motor")  # Nama perangkat motor kiri
right_motor = robot.getDevice("right wheel motor")  # Nama perangkat motor kanan
...
def compute_speeds(us_values):
    ...
    for i in range(2):
        for k in range(2):
            speed[i] += us_values[k] * coefficients[i][k]  # Memperbaiki sintaksis
    return speed
